...
export function workbookToRows(workbook) {
    ...
    worksheet.eachRow((row) => {
        ...
        for (let col = 1; col <= maxCols; col++) {
            const cellVal = row.getCell(col).text ?? ""; //getCell(<kolumn>) är också en inbyggd ExcelJS-metod, om det ligger något annat än en sträng i cellen så kommer .text att returnera den rimligaste tolkningen i text, om jag har förstått det rätt. Nullish coalescing operator (= ??) returnerar det vänstra värdet om inte det är null eller undefined, annars skickar den tillbaka det högra värdet (i det här fallet en tom sträng). 
